...
void R_LoadEntities( lump_t *l ) {
	...
	while ( 1 ) {	
		...
		char const* vertexremapshaderstr = "vertexremapshader";
		if (!Q_strncmp(keyname, vertexremapshaderstr, sizeof(vertexremapshaderstr)-1) ) {
			s = strchr(value, ';');
			if (!s) {
		...
		char const* remapshaderstr = "remapshader";
		if (!Q_strncmp(keyname, remapshaderstr, sizeof(remapshaderstr)-1) ) {
			s = strchr(value, ';');
			if (!s) {
